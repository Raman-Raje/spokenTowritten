...
class Text2Digits():
    ...
    def convert_to_digits(self, textnum, spell_check=False):
        ...
        current = result = word_count = multiple = 0
        word_prev = ''
        curstring = ''
        ...
        for word in textnum.split():
            ...
            # added support for nought. Need to modify it.
            word = 'zero' if word in ["nought","naught"] else word
            ...
            if word in ORDINAL_WORDS:
                ...
                multiple = MULTIPLES[word]
                word_prev = word_original
                ...
                if (not self.is_numword(word)) or (word == 'and' and not lastscale):
                   ...
                   # to handle cases such as double and triple
                    if multiple:
                        word_original = word_original*multiple if (word in self.accepted) and len(word) == 1 else word_prev+" "+word_original
                ...
                # Is a number word
                    ...
                    if multiple:
                        current = int(str(current)*multiple)
                        multiple = 0
